(function (exports) {
	...
	function serialize(obj, indent) {
		...
		if (obj instanceof Array) {
			...
			array of non-objects -> [1, 2, 3, 4, 5]
			*/
			if (obj.length === 1 || !obj.some(isObject)) {
				return '[' + obj.map(function (item) { return serialize(item, indent) }).join(', ') + ']';
			}
			...
			*/
			...
			return '[\n' + obj.map(function (item) { return localIndent + serialize(item, localIndent) }).join(',\n') + '\n' + indent + ']';
		}
		...
		/*
		object of 0, 1 or 2 non-object values -> {x: 1, y: 2}
		*/
		if (pairs.length <= 2 && !pairs.some(function (pair) { return isObject(pair.value) })) {
			return '{' + pairs.map(function (pair) {
				return serializeKey(pair.key) + ': ' + JSON.stringify(pair.value);
			}).join(', ') + '}';
		}
		...
		*/
		...
		return '{\n' + pairs.map(function (pair) {
			return localIndent + serializeKey(pair.key) + ': ' + serialize(pair.value, localIndent);
		}).join(',\n') + '\n' + indent + '}';
	}
